...
class ProGenAttention(nn.Module):
    def __init__(self, config):
        ...
        # update_chq: adding position embeddings that supports longer inputs
        # self.register_buffer(
        #     "causal_mask",
        #     torch.tril(torch.ones((10000, 10000), dtype=torch.bool))
        #             .view(1, 1, 10000, 10000)
        # )
    ...
    def _attn(
        ...
        # update_chq: adjust causal mask to support longer inputs
        # causal_mask = self.causal_mask[:, :, :query_length, :query_length].bool()
...
class ProGenModel(ProGenPreTrainedModel):
    def __init__(self, config):
        ...
        # update_chq: adding position embeddings that supports longer inputs
        # self.reinit_position_embeddings(10000)
    ...
    def reinit_position_embeddings(self, max_position_embeddings):
        self.wpe = nn.Embedding(max_position_embeddings, self.config.n_embd)
...
def ProGenTokenizer(dict_path="tokenizer.json"):
    ## init tokenizer ##
